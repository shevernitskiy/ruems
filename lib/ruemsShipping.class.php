...
class ruemsShipping extends waShipping
{
    ...
    public function calculate()
    {
        ...
        require_once dirname(__FILE__).'/classes/shippingRuemsTariffRP.class.php';
        try {
            $t = new shippingRuemsTariffRP();
        ...
        if ($this->getSettings('isestdate')) {
            ...
            $services[0]['est_delivery'] = waDateTime::format('humandate', strtotime('now') + ($this->getEstDate($this->getAddress('country'),'min') + $this->getSettings('adddays'))*24*60*60).' - '.waDateTime::format('humandate', strtotime('now') + ($this->getEstDate($this->getAddress('country'),'max') + $this->getSettings('adddays'))*24*60*60);
        }
    ...
    {
        require_once dirname(__FILE__).'/classes/shippingRuemsPrintRP.class.php';

        $pf = new shippingRuemsPrintRP();
    ...
    public function getEstDate($code,$type = min)
    {
        ...
        switch ($type) {
            case 'min':
                $sar = array_column($ar, 'minDays', 'code3');
                break;
            case 'max':
                $sar = array_column($ar, 'maxDays', 'code3');
                break;
            default:
                return 0;
                break;
    ...
    {
        if ($tracking_id) {
            require_once dirname(__FILE__).'/model/shippingRuems.model.php';
            $model = new waModel();
            try {
